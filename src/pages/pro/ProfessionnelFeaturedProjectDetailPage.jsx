...
async function fetchPublicProfessionalBySlug(slug) {
  const sources = ["professionnels_public", "professionnels_partner_page_public"];
  ...
  for (const table of sources) {
    ...
    if (error) {
      console.error(`[ProfessionnelFeaturedProjectsPage] source error (${table})`, error);
      continue;
    }
...
async function fetchFeaturedProjectsForPro(professionnelId) {
  ...
  const buyMap = new Map((buyRes.data || []).map((p) => [p.id, { ...p, type_projet: "achat" }]));
  const sellMap = new Map((sellRes.data || []).map((p) => [p.id, { ...p, type_projet: "vente" }]));
...
const ProfessionnelFeaturedProjectsPage = () => {
  ...
  const isFromDigitalCard =
    searchParams.get("cvd") === "1" || searchParams.get("entry") === "external";
  ...
  const handleOpenProject = (project) => {
    ...
    navigate(
      `/pro/${encodeURIComponent(resolvedSlug)}/opportunites/${encodeURIComponent(tx)}/${encodeURIComponent(id)}?${sp.toString()}`,
      {
        state: {
  ...
  if (!professional) {
    return (
      <div className="flex min-h-[60vh] flex-1 flex-col items-center justify-center px-4 text-center">
        <h2 className="mb-2 text-2xl font-bold text-slate-800">Professionnel introuvable</h2>
            ...
            {projects.length === 0 ? (
              <div className="rounded-xl border border-slate-200 bg-white p-12 text-center shadow-sm">
                ...
                <p className="mx-auto max-w-md text-slate-500">
                  Ce professionnel n&apos;a pas encore ajouté de projets à sa sélection.
                </p>
              </div>
            ) : (
              <motion.div
