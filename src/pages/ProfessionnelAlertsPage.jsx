// src/pages/ProfessionnelAlertsPage.jsx
    ...
    <div className="grid grid-cols-1 md:grid-cols-12 gap-3">
      <div className="md:col-span-5 space-y-1">
        <Label className="flex items-center">
          <MapPin className="mr-2 h-4 w-4" /> Ville
        </Label>
        <Input
          list={`cities-${row.id}`}
        ...
        {loadingCity && (
          <p className="text-xs text-muted-foreground">Recherche des villes…</p>
        )}
      </div>
...
const toChoiceColumns = (locations = []) => {
  ...
  return {
    city_choice_1: cities[0],
    city_choice_2: cities[1],
    city_choice_3: cities[2],
    city_choice_4: cities[3],
    city_choice_5: cities[4],
    quartier_choice_1: quartiers[0],
    quartier_choice_2: quartiers[1],
    quartier_choice_3: quartiers[2],
    quartier_choice_4: quartiers[3],
    quartier_choice_5: quartiers[4],
    department_choice_1: depts[0],
    department_choice_2: depts[1],
    department_choice_3: depts[2],
    department_choice_4: depts[3],
    department_choice_5: depts[4],
    region_choice_1: regions[0],
    region_choice_2: regions[1],
    region_choice_3: regions[2],
    region_choice_4: regions[3],
    region_choice_5: regions[4],
  };
};
...
const AlertForm = ({ alertToEdit, onSave, onCancel }) => {
    ...
    if (alertToEdit) {
      const locations =
        Array.isArray(alertToEdit.locations) && alertToEdit.locations.length
          ? alertToEdit.locations.map((loc, i) => ({
              id: Date.now() + i,
              city: '',
              quartier: '',
              department: '',
  ...
  const changeLocationRow = (updatedRow) => {
    setFormData((prev) => ({
      ...
      locations: prev.locations.map((l) =>
        l.id === updatedRow.id ? updatedRow : l
      ),
    }));
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (error) {
      ...
      toast({
        title: 'Succès',
        description: 'Alerte sauvegardée avec succès.',
      });
      onSave();
    }
    setLoading(false);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="space-y-2">
          ...
          <Select
            ...
            onValueChange={(value) =>
              setFormData((p) => ({ ...p, type_projet: value }))
            }
          >
            <SelectTrigger>
          ...
          <p className="text-xs text-muted-foreground">
            {isAchat
              ...
              : 'Mise en vente d’un bien (plage de prix attendue)'}
          </p>
        </div>

        <div className="space-y-2">
          <Label className="flex items-center">
            <CalendarDays className="mr-2 h-4 w-4" /> Échéance
          </Label>
          <Select
            value={validateSelectValue(formData.delai)}
            onValueChange={(value) =>
              setFormData((p) => ({ ...p, delai: value }))
            }
          >
            <SelectTrigger>
      ...
      <div className="space-y-3">
        <Label className="flex items-center">
          <Building className="mr-2 h-4 w-4" /> Type de bien
        </Label>
      ...
      <div className="space-y-2">
        <Label className="flex items-center">
          <MapPin className="mr-2 h-4 w-4" /> Localisations
        </Label>
        <div className="space-y-3">
      ...
      <div className="space-y-3">
        <Label className="flex items-center">
          <ThumbsUp className="mr-2 h-4 w-4" /> Caractéristiques obligatoires
        </Label>
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
...
const AlertCard = ({
  ...
  onViewMatchingProjects,
}) => {
  const delaiLabel =
    delaiOptions.find((d) => d.value === alert.delai)?.label || null;

  const locs =
    Array.isArray(alert.locations) && alert.locations.length
      ? alert.locations
  ...
  const priceDisplay =
    alert.type_projet === 'vente'
      ? [alert.prix_demande_min, alert.prix_demande_max]
          .filter(Boolean)
          .join(' - ') + ' €'
  ...
  const criteria = [
    {
      icon: Building,
      value: Array.isArray(alert.type_bien)
        ? alert.type_bien.join(', ')
        : alert.type_bien,
    },
    ...
    {
      icon: BedDouble,
      value: alert.bedrooms_min ? `${alert.bedrooms_min}+ ch` : null,
    },
  ].filter(
    (c) =>
      c.value &&
  ...
  return (
    <Card className="flex flex-col">
          ...
          <div className="flex flex-wrap gap-1 pt-2">
            {alert.required_features.map((f) => (
              <Badge key={f} variant="secondary">
                {featureOptions.find((opt) => opt.id === f)?.label || f}
              </Badge>
            ))}
          </div>
        )}
      ...
      <CardFooter className="border-t pt-4 flex-col items-stretch space-y-2">
        <Button
          onClick={() => onViewMatchingProjects(alert)}
          disabled={matchingProjectsCount === 0}
        >
...
const projectMatchesAlert = (project, alert) => {
  ...
  if (alert.type_projet && norm(alert.type_projet) !== norm(project.type_projet))
    return false;

  const alertTypesArray = Array.isArray(alert.type_bien)
    ? alert.type_bien.filter(Boolean)
    : alert.type_bien
    ? [alert.type_bien]
    : [];
  if (alertTypesArray.length > 0) {
    const projectTypeBien = norm(project.type_bien);
    if (!projectTypeBien || !alertTypesArray.map(norm).includes(projectTypeBien))
      return false;
  }
  ...
  const alertCities = locs
    .map((l) => l?.city)
    .filter(Boolean)
    .map(norm);
  const alertQuartiers = locs
    .map((l) => l?.quartier)
    .filter(Boolean)
    .map(norm);
  ...
  if (alertCities.length > 0) {
    const projectCity =
      norm(project?.location?.city) || norm(project?.city_choice_1) || '';
    if (!projectCity || !alertCities.includes(projectCity)) return false;
  }
  if (alertQuartiers.length > 0) {
    const projectQuartier =
      norm(project?.location?.quartier) || norm(project?.quartier) || '';
    if (projectQuartier && !alertQuartiers.includes(projectQuartier))
      return false;
  }
  ...
  const reqFeatures = Array.isArray(alert.required_features)
    ? alert.required_features
    : [];
...
const ProfessionnelAlertsPage = () => {
  ...
  // lecture de access_plan via la vue professionnels_billing_status
  useEffect(() => {
    if (!user) return;
    ...
    const fetchSubscription = async () => {
      ...
      try {
        ...
        if (error) {
          console.error('Erreur récupération access_plan', error);
          setIsFreePlan(false);
          return;
        }

        setIsFreePlan(data?.access_plan === 'free');
      } catch (e) {
        console.error('Erreur inattendue abonnement pro', e);
        setIsFreePlan(false);
      } finally {
  ...
  const handleOpenForm = (alert = null) => {
    // ✅ Blocage création uniquement pour les plans "free".
    // - alert === null  => création → interdit si free
    // - alert !== null  => modification → autorisée même si free
    if (isFreePlan && !alert) {
      toast({
        variant: 'destructive',
        title: 'Création d’alertes réservée aux offres payantes',
        description:
          "Votre plan d’accès est 'Gratuit'. Vous pouvez consulter et modifier vos alertes existantes, mais la création de nouvelles alertes est réservée aux offres supérieures (ex : Premium Plus).",
      });
      return;
    }
  ...
  const handleToggleStatus = async (alert) => {
    ...
    if (error) {
      ...
      toast({
        title: 'Succès',
        description: `Alerte ${newStatus ? 'activée' : 'désactivée'}.`,
      });
      fetchData();
    }
  };
  ...
  return (
    <>
      <SEO
        title="Mes Alertes Projet"
        description="Paramétrez des alertes pour être informé des nouveaux projets immobiliers."
      />
      <div className="container mx-auto px-4 py-10">
        <div className="flex justify-between items-center mb-8">
          <div>
            ...
            {!planLoading && isFreePlan && (
              <p className="mt-1 text-sm text-amber-700 bg-amber-50 border border-amber-200 px-3 py-2 rounded-md">
                Votre plan d’accès est <span className="font-semibold">Gratuit</span> : 
                vous pouvez consulter et modifier vos alertes existantes, 
                mais la création de nouvelles alertes est réservée aux offres supérieures.
              </p>
            )}
          </div>
          ...
          {!planLoading &&
            (isFreePlan ? (
              <Button asChild variant="outline">
                <Link to="/professionnel/abonnements">
              ...
              <Button onClick={() => handleOpenForm()}>
                <PlusCircle className="mr-2 h-4 w-4" /> Créer une alerte
              </Button>
            ))}
        </div>
        ...
        {loading ? (
            ...
            {isFreePlan ? (
              <>
                <p className="text-lg text-muted-foreground">
                  Avec votre plan gratuit, vous ne pouvez pas encore créer d’alertes projet.
                </p>
                <p className="mt-2 text-sm text-muted-foreground max-w-xl mx-auto">
                  Passez sur une offre supérieure pour recevoir automatiquement les nouveaux projets
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {alerts.map((alert) => {
              const matchingCount = allProjects.filter((p) =>
        ...
        {/* Le formulaire n'est ouvert que si on a cliqué + que la garde handleOpenForm l’autorise */}
        <Dialog open={isFormOpen} onOpenChange={setIsFormOpen}>
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>
                {editingAlert ? 'Modifier' : 'Créer'} une alerte
              </DialogTitle>
              <DialogDescription>
                Définissez vos critères pour recevoir des notifications sur les
                nouveaux projets.
              </DialogDescription>
            </DialogHeader>
            <div className="py-4">
        ...
        >
          <DialogContent className="max-w-6xl max-h-[90vh] flex flex-col">
            <DialogHeader>
              <DialogTitle>
                Projets correspondants à "{selectedAlertName}"
              </DialogTitle>
              <DialogDescription>
                Voici les projets qui correspondent aux critères de votre
                alerte.
              </DialogDescription>
            </DialogHeader>
...
export default ProfessionnelAlertsPage;